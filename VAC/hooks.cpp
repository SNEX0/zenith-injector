...
{
	auto* const h_steamservice = GetModuleHandleA(xorstr_("steamservice.dll") );
	...
	// LOADING
	{
		...
		auto* const dw_loading = memory::pattern_scan_ida( h_steamservice, xorstr_("55 8B EC 83 EC 28 53 56 8B 75 08 8B") );
		if ( !dw_loading )
		{
			valve::msg(xorstr_("[ VAC3 ] dw_loading is nullptr...\n") );
			return false;
		}
	...
	// CALLING
	{
		...
		auto* const dw_calling = memory::pattern_scan_ida( h_steamservice, xorstr_("55 8B EC 6A FF 68 ? ? ? ? 68 ? ? ? ? 64 A1 ? ? ? ? 50 64 89 25 ? ? ? ? 83 EC 6C 53 56") );
		if ( !dw_calling )
		{
			valve::msg(xorstr_("[ VAC3 ] dw_calling is nullptr...\n") );
			return false;
		}
...
bool __stdcall hooks::hk_loading( valve::vac_buffer* h_mod, char injection_flags )
{
	...
	if ( nt_header_crc32 )
	{
		valve::msg( xorstr_("[ VAC3 ] loading module crc32 [ 0x%.8X ].\n"), nt_header_crc32 );
